...
import { formatRupiah, remainingOf, paidAmountOf } from "../lib/debt-utils";

// Modal rincian satu transaksi belanja (piutang) yang masih berjalan: daftar
// barang beserta progress bayar & riwayat cicilan per barang, plus tombol
// bayar per barang dan "Tandai transaksi ini lunas". Murni presentasional —
// semua aksi (bayar, hapus barang, tandai lunas) didelegasikan ke pemanggil.
//
// Props:
// - group: { trxNo, date, kasir, items } — grup transaksi yang ditampilkan
// - onClose(): dipanggil saat modal ditutup
// - onPayItem(item, mode): dipanggil saat tombol "Bayar sebagian"/"Tandai lunas" per barang diklik
// - onDeleteItem(itemId): dipanggil saat "Hapus barang ini" diklik
// - onMarkGroupPaid(items): dipanggil saat "Tandai transaksi ini lunas" diklik
export default function TransactionDetailModal({ group, onClose, onPayItem, onDeleteItem, onMarkGroupPaid }) {
      ...
      <div className="bg-[var(--card)] rounded-t-3xl sm:rounded-3xl w-full sm:max-w-sm max-h-[88vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-[var(--card)] px-5 pt-5 pb-4 border-b border-[var(--paper-line)] flex items-start justify-between gap-3 z-10">
          <div className="min-w-0">
            ...
            <div className="font-ledger text-lg mt-0.5 font-mono-num">{group.trxNo}</div>
            ...
            return (
              <div key={it.id} className="rounded-2xl border border-[var(--paper-line)] p-3.5">
                ...
                <div className="flex gap-2 mt-2.5">
                  <button onClick={() => onPayItem(it, "partial")} className="flex-1 py-1.5 rounded-lg border border-[var(--paper-line)] text-xs font-medium">
                    Bayar sebagian
                  </button>
                  <button onClick={() => onPayItem(it, "lunas")} className="flex-1 py-1.5 rounded-lg bg-[var(--green)] text-white text-xs font-medium">
                    Tandai lunas
                  </button>
                </div>
                ...
                <div onClick={() => onDeleteItem(it.id)} className="text-[11px] text-[var(--ink-soft)] underline cursor-pointer mt-2 inline-block">
                  Hapus barang ini
                </div>
              </div>
            );
        ...
        <div className="px-5 pb-5 pt-3 border-t border-[var(--paper-line)] sticky bottom-0 bg-[var(--card)]">
          <div className="flex justify-between items-baseline mb-3">
            ...
            <span className="font-mono-num text-xl font-bold">{formatRupiah(total)}</span>
          </div>
          <button
            onClick={() => onMarkGroupPaid(group.items)}
